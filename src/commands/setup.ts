...
export default class Setup extends Command {
  ...
  updateConfig(platform: string, projectName: string, projectDir: string, unrealDir: string, buildsDir: string, artifactsDir: string, clientS3Bucket: string, serverS3Bucket: string, config: any): {} {
  ...
  async run() {
    try {
      ...
      const exampleArtifactsDir: string = platform === PLATFORMS.MAC ?
        ...
        : 'C:\\Artifacts'
      const exampleClientS3Bucket: string = 'client.magnumopus.gg'
      const exampleServerS3Bucket: string = 'server.magnumopus.gg'
